...
- Store planner/tool traces on chat messages or sessions so failures can be debugged and evaluated. Current traces include `plannerSource` (`ai_planner` or `deterministic`), model, rationale, chosen tools, context strategy, source scope, context summary, and a user-facing context coverage label such as `exam scope coverage`, `lecture-order lesson pack`, or `focused source context`.
...
- Parent-child retrieval. Initial v0 is implemented for focused Chat retrieval: search still ranks small passages, then generation context expands each selected passage with neighboring source-order chunks before packing.
